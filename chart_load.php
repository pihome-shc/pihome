...
<?php
    ...
    while ($row = mysqli_fetch_assoc($resulta)) {
        ...
        while ($rowb = mysqli_fetch_assoc($result)) { 
			...
			//$bat_voltage[] = array(strtotime($rowb['update']) * 1000, $rowb['bat_voltage']);
        }
        // create dataset entry using distinct color based on zone index(to have the same color everytime chart is opened)
        echo "{label: \"".$label."\", data: ".json_encode($bat_level).", color: rainbow(".$count.",".++$counter.") }, \n";
		//echo "{label: \"".$node_id."\", data: ".json_encode($bat_voltage).", color: rainbow(".$count.",".++$counter.") }, \n";
    }
?> ];
var options_bat = {
	xaxis: { mode: "time", timezone: "browser", timeformat: "%b %Y"},
...
<?php
    ...
    while ($row = mysqli_fetch_assoc($resulta)) {
        ...
        // create dataset entry using distinct color based on zone index(to have the same color everytime chart is opened)
        //echo "{label: \"".$node_id."\", data: ".json_encode($bat_level).", color: rainbow(".$count.",".++$counter.") }, \n";
		echo "{label: \"".$label."\", data: ".json_encode($bat_voltage).", color: rainbow(".$count.",".++$counter.") }, \n";
    }
?> ];
...
$(document).ready(function () {$.plot($("#battery_voltage"), bat_voltage_dataset, options_bat_volt);$("#battery_voltage").UseTooltipv();});
var previousPoint = null, previousLabel = null;
$.fn.UseTooltipv = function () {
    $(this).bind("plothover", function (event, pos, item) {
        if (item) {
            if ((previousLabel != item.series.label) ||
                 (previousPoint != item.dataIndex)) {
                ...
                showTooltip(item.pageX,
                        item.pageY,
                        color,
                        "<strong>" + item.series.label + "</strong> At: " + (new Date(x).getHours()<10?'0':'') + new Date(x).getHours() + ":"  + (new Date(x).getMinutes()<10?'0':'') + new Date(x).getMinutes() +"<br> <strong><?php echo $lang['battery_voltage']; ?>  : " + $.formatNumber(y, { format: "#,###", locale: "us" }) + "v</strong> ");
            }
        } else {
            $("#tooltip").remove();
            previousPoint = null;
