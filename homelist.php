...
while ($row = mysql_fetch_assoc($results)) {
	...
	$result = mysql_query($query, $connection);
	$roomtemp = mysql_fetch_array($result);
	...
	$sch_results = mysql_query($query, $connection);
	$schedule = mysql_fetch_array($sch_results);
	...
	*/
	...
	elseif(($room_c > $frost_c) && ($room_c < $max_room_c)){
		if ($away_active=='0'){	
			if($bactive=='0'){
				...
				if (mysql_num_rows($sch_results) != 0){
					$status="0";
	...
	$sresults = mysql_query($squery, $connection);
	if (mysql_num_rows($sresults) == 0){
		...
		echo '<h4>'.mysql_num_rows($sresults).' Schedule Records found.</h4><br>
...
$result = mysql_query($query, $connection);
$boiler_onoff = mysql_fetch_array($result);
...
echo '</h3></button>';
	...
	$bresults = mysql_query($bquery, $connection);
	if (mysql_num_rows($bresults) == 0){
		...
		echo '<p class="text-muted">'. mysql_num_rows($bresults) .' Last Boiler Log Records. </p>
		<div class=\"list-group\">' ;
		...
		while ($brow = mysql_fetch_assoc($bresults)) {
...
$result = mysql_query($query, $connection);
//confirm_query($result);
$boiler_time = mysql_fetch_array($result);
